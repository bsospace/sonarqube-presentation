...
// ❌ Vulnerability: ใช้คีย์เข้ารหัสที่สั้นเกินไป
...
// ❌ Vulnerability: ใช้ AES โดยไม่มี padding scheme ที่ปลอดภัย
const insecureCipher = crypto.createCipheriv('aes-128-ecb', weakKey, null); // ❌ ECB mode ไม่ปลอดภัย
let insecureEncrypted = insecureCipher.update("sensitive-data", 'utf8', 'hex');
insecureEncrypted += insecureCipher.final('hex');
console.log(insecureEncrypted);

// 7. File uploads should be restricted
...
// 8. JWT should be signed and verified with strong cipher algorithms
...
// 9. Origins should be verified during cross-origin communications
